 ...
 *
 * The /api/pull endpoint returns a newline-delimited JSON stream.
 * We must consume the response body for the pull to actually complete.
 */
async function pullOllamaModel(
  ...
  // Consume the streaming response — the pull won't complete otherwise.
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          ...
          // Skip lines that aren't valid JSON (e.g. empty lines)
        }
      }
    }
  }
      ...
      try {
        ...
        if (providerName === "ollama") {
          try {
            ...
            await pullOllamaModel(id, pullBaseUrl, signal, (status) => {
              _onUpdate?.({ content: [{ type: "text", text: `Pulling ${id}: ${status}...` }] });
            });
          } catch (pullErr) {
